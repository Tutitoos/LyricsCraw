...
func ScrapeVagalume(query string) (string, error) {
	...
	fmt.Println("Step 2: Extracted href:", href)

	// Remove "-traducao" from the href
	if strings.Contains(href, "-traducao") {
		href = strings.ReplaceAll(href, "-traducao", "")
	}
	fmt.Println("Step 3: Processed href (removed -traducao if present):", href)
	...
	fmt.Println("Step 4: Navigated to href:", href)
	...
	if err != nil {
		fmt.Printf("Step 5: Failed to extract vData, proceeding with modal check: %v\n", err)
	} else if !strings.Contains(vData, `hasBadwords:1`) {
		fmt.Println("Step 5: No explicit content (hasBadwords:0), skipping modal check")
	} else {
		...
		if err == nil {
			...
			fmt.Println("Step 5: Clicked age restriction button")
		} else {
			...
			if err == nil {
				...
				fmt.Println("Step 5: Clicked fallback age restriction button")
			} else {
				...
				fmt.Printf("Step 5: Age restriction modal button (%s) and fallback (%s) not found, proceeding without click. Error: %v\nBody HTML (truncated): %s\n", modalSelector, fallbackSelector, err, bodyHTML[:500])
			}
		}
	}
	...
	fmt.Println("Step 6: Extracted lyrics content")
...
SOU MAIOR DE 18 ANOS`
	...
	fmt.Println("Step 7: Cleaned lyrics content")
